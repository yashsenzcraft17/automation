...
class PostgreSQLConnector:
    ...
    def create_lead_request_table(self):
        ...
        CREATE TABLE IF NOT EXISTS lead_request (
            lead_request_detail_id SERIAL PRIMARY KEY,
            lead_channel_id INT,
    ...
    def insert_lead_request(self, lead_data):
        ...
        INSERT INTO lead_request (
            ...
            country, designation, department, location, contacted_on, lead_status_id,
            reviewed_by, reviewed_on, is_auto_approved, is_posted, created_by, created_on
        ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        RETURNING lead_request_detail_id;
        """
        ...
        lead_values = (
            ...
            str(lead_data.get("Phone", "")[:45]),  # Truncate to 45 characters
            mobile_value,  # Use the value obtained above
            str(lead_data.get("Email", "")[:45]),  # Truncate to 45 characters
            ...
            str(lead_data.get("Location")[:45]) if lead_data.get("Location") else None,  # Truncate to 45 characters
            datetime.now(),
            lead_data.get("lead_status_id"),
        ...
        lead_request_detail_id = self.cursor.fetchone()[0]
        self.conn.commit()

        return lead_request_detail_id
...
if __name__ == "__main__":
    ...
    postgres_connector = PostgreSQLConnector(**db_params)
    postgres_connector.create_lead_request_table()

    # ... (your existing code)

    lead_data = {
        "lead_channel_id": 1,  # Replace with actual lead_channel_id
        "lead_record_type": "SomeRecordType",  # Replace with actual lead_record_type
        ...
        "lead_status_id": 1,  # Replace with actual lead_status_id
    ...
    lead_request_detail_id = postgres_connector.insert_lead_request(lead_data)
